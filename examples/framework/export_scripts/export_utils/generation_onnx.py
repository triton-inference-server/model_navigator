...
# Based on https://github.com/huggingface/transformers/blob/main/examples/research_projects/onnx/summarization/bart_onnx/generation_onnx.py
...
class BARTGenerator(torch.nn.Module, GenerationMixin):
    ...
    def _prepare_decoder_input_ids_for_generation(
        ...
        decoder_input_ids = (
            torch.ones((input_ids.shape[0], 1), dtype=input_ids.dtype, device=input_ids.device) * decoder_start_token_id
        )
...
class BARTBeamSearchGenerator(BARTGenerator):
    ...
    def forward(
        ...
        decoder_start_token_id = decoder_start_token_id.item()
