...
def tensorflow_export_builder(config: Config, package_descriptor: "PackageDescriptor") -> Pipeline:
    ...
    export_savedmodel = ExportTF2SavedModel()
    commands.append(export_savedmodel)
    ...
    if Format.TENSORRT in config.target_formats:
        for target_precision in config.target_precisions:
            commands.append(ConvertONNX2TRT(target_precision=target_precision, requires=(onnx_convert,)))

    if Format.TF_TRT in config.target_formats:
        for target_precision in config.target_precisions:
            commands.append(ConvertSavedModel2TFTRT(target_precision=target_precision, requires=(export_savedmodel,)))

    return Pipeline(name="TensorFlow 2 Export", framework=Framework.TF2, commands=commands)
