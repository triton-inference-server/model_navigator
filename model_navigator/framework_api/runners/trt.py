...
class TrtRunner(INavigatorRunner, _TrtRunner):
    ...
    def infer(self, feed_dict, check_inputs=None, *args, **kwargs):
        feed_dict = {
            name: tensorrt.cast_tensor(tensor)
            for name, tensor in feed_dict.items()
            if name in self.get_input_metadata()
        }
...
class TrtexecRunner(INavigatorStabilizedRunner, _TrtexecRunner):
    ...
    def read_output_file(self):
        ...
        outputs = OrderedDict()
        # Open file is required here to reset the pointer to the start of the file
        # TODO: Move this part to polygraphy_trtexec
        with open(self.export_output_file_handle.name) as fp:
