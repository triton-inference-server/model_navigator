...
class GPUDeviceFactory:
    ...
    def create_device_by_bus_id(bus_id, dcgmPath=None):
        ...
        dcgm_handle = dcgm_agent.dcgmStartEmbedded(structs.DCGM_OPERATION_MODE_MANUAL)
        gpu_devices = dcgm_agent.dcgmGetAllSupportedDevices(dcgm_handle)
        for gpu_device in gpu_devices:
            device_atrributes = dcgm_agent.dcgmGetDeviceAttributes(dcgm_handle, gpu_device).identifiers
            pci_bus_id = bytes(device_atrributes.pciBusId.decode("ascii").upper(), encoding="ascii")
            ...
            dcgm_agent.dcgmShutdown()
            raise ModelNavigatorException(f"GPU with {bus_id} bus id is not supported by DCGM.")
    ...
    def create_device_by_cuda_index(index):
        ...
        return GPUDeviceFactory.create_device_by_bus_id(bytes(device_bus_id, encoding="ascii"))
    ...
    def create_device_by_uuid(uuid, dcgmPath=None):
        ...
        for gpu_device in gpu_devices:
            ...
            dcgm_agent.dcgmShutdown()
            raise ModelNavigatorException(f"GPU UUID {uuid} was not found.")
