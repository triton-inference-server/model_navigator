            ...
            if dcgmLib is None:
                try:
                    if sys.platform[:3] == "win":
                        ...
                        dcgmLib = CDLL(
                            os.path.join(
                                os.getenv("ProgramFiles", "C:/Program Files"), "NVIDIA Corporation/NVSMI/dcgm.dll"
                            )
                        )
                    else:
                        if not libDcgmPath:
...
class c_dcgmClockSet_v1(_PrintableStructure):
    _fields_ = [("version", c_uint), ("memClock", c_uint), ("smClock", c_uint)]  # Memory Clock  # SM Clock
...
class c_dcgmDeviceSupportedClockSets_v1(_PrintableStructure):
    ...
    _fields_ = [("version", c_uint), ("count", c_uint), ("clockSet", c_dcgmClockSet_v1 * DCGM_MAX_CLOCKS)]
...
class c_dcgmDeviceMemoryUsage_v1(_PrintableStructure):
    ...
    _fields_ = [("version", c_uint), ("bar1Total", c_uint), ("fbTotal", c_uint), ("fbUsed", c_uint), ("fbFree", c_uint)]
...
class c_dcgmDeviceEncStats_v1(_PrintableStructure):
    _fields_ = [("version", c_uint), ("sessionCount", c_uint), ("averageFps", c_uint), ("averageLatency", c_uint)]
...
class c_dcgmPolicyConditionXID_t(_PrintableStructure):
    _fields_ = [("timestamp", c_longlong), ("errnum", c_uint)]  # timestamp of the error  # XID error number
...
class c_dcgmPolicyCallbackResponse_v1(_PrintableStructure):
    ...
    _fields_ = [("version", c_uint), ("condition", c_int), ("val", Value)]  # an OR'ed list of DCGM_POLICY_COND_*
...
class c_dcgmDiagTestResult_v2(_PrintableStructure):
    _fields_ = [("result", c_uint), ("error", c_dcgmDiagErrorDetail_t), ("info", c_char * 1024)]
...
dcgmIntrospectFullFieldsExecTime_version2 = make_dcgm_version(c_dcgmIntrospectFullFieldsExecTime_v2, 2)
...
class c_dcgmRunDiag_v6(_PrintableStructure):
    _fields_ = [
        ...
        ("goldenValuesFile", c_char * DCGM_PATH_LEN),  # The path where the golden values should be recorded
        (
...
class c_dcgmNvLinkStatus_v1(_PrintableStructure):
    ...
    _fields_ = [
        ...
        ("gpus", c_dcgmNvLinkGpuLinkStatus_v1 * DCGM_MAX_NUM_DEVICES),  # Per-GPU NvLink link statuses
        ("numNvSwitches", c_uint32),  # Number of NvSwitches populated in nvSwitches[]
        ("nvSwitches", c_dcgmNvLinkNvSwitchLinkStatus_t * DCGM_MAX_NUM_SWITCHES),  # Per-NvSwitch NvLink link statuses
    ]
