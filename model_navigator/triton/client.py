...
from model_navigator.triton.exceptions import TritonServerNotReadyException
...
class TritonClient:
    ...
    >>> for inference_data in client.run_inference(model_name="ResNet50",
    >>>                                            model_version="1", dataloader=dataloader)
    >>>     inference_data.y_pred
    ...
    @property
    def protocol(self) -> TritonClientProtocol:
        """Return protocol with which client communicates with Triton.

        Returns:
            Protocol with which client communicates with Triton.
        """
        protocol, *_ = parse_server_url(self.server_url)
        return TritonClientProtocol(protocol)
    ...
    def wait_for_server_ready(self, timeout: int):
        ...
        raise TritonServerNotReadyException("Could not determine server readiness. " "Number of retries exceeded.")
    ...
    def wait_for_model(
        self, *, model_name: str, model_version: str, timeout_s: int = 120, check_interval_s: int = 5
    ) -> Dict[str, Any]:
        ...
        while elapsed_time_s < timeout_s and _shall_wait(state):
            LOGGER.info(f"waiting for model... {elapsed_time_s:.0f}/{timeout_s} state={state}")
            time.sleep(check_interval_s)
            state = self.get_model_state(model_name, model_version)
            elapsed_time_s = time.time() - start_time_s

        if not self.client.is_model_ready(model_name):
            raise RuntimeError(f"Model {model_name} requested to be loaded, but is not ready")
    ...
    def _format_response(self, response):
        if not isinstance(response, dict):
            response = json.loads(grpc_client.MessageToJson(response, preserving_proto_field_name=True))
        return response

    def _create_client(self, server_url: str, verbose: bool):
        protocol, host, port = parse_server_url(server_url)
        if self.protocol == TritonClientProtocol.HTTP and http_client is None:
            raise RuntimeError(
        ...
        client_lib = {TritonClientProtocol.HTTP: http_client, TritonClientProtocol.GRPC: grpc_client}[self.protocol]
