...
def _default_validator(x, value):
    ...
    return ConfigStatus(CONFIG_PARSER_FAILURE, f'The value for field "{value.name()}" should not be empty.')
...
class ConfigPrimitive(ConfigValue):
    ...
    def set_value(self, value):
        ...
        if self._is_primitive(value):
            ...
            return ConfigStatus(
                CONFIG_PARSER_FAILURE, f'Value "{value}" for field "{self.name()}" should be a primitive type.', self
            )
