...
def _default_validator(x, value):
    ...
    return ConfigStatus(
        ...
        f'The value for field "{value.name()}" should be a dictionary' " and the length must be larger than zero.",
    )
...
class ConfigObject(ConfigValue):
    ...
    def __init__(self, schema, preprocess=None, required=False, validator=None, output_mapper=None, name=None):
        """
        schema : dict
        ...
        if type(value) is dict:
            for key, value_ in value.items():
                ...
                # present, we use the schema for the wildcard.
                    ...
                    return ConfigStatus(
                        CONFIG_PARSER_FAILURE,
                        f'Key "{key}" should not be ' f'specified in field "{self.name()}".',
                        self,
                    )
            ...
            return ConfigStatus(
                CONFIG_PARSER_FAILURE,
                f'Value for field "{self.name()}" should be an object.' f" Type {type(value)} is provided instead.",
                self,
            )
