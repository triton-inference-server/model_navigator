...
```shell
$ pip install --extra-index-url https://pypi.ngc.nvidia.com git+https://github.com/triton-inference-server/model_navigator.git@vx.x.x [<extras,>] --upgrade
```
...
### Navigator package
Export results are stored inside ```navigator_workdir``` directory in ```<model_name>.nav.workspace``` directory. Saved results are stored as a ```.nav``` package at the path specified by the user.
