...
from model_navigator.api.config import DeviceKind, Format, JitType, OptimizationProfile
...
def test_torch_export_builder_does_not_return_graph_surgeon_optimization_when_disabled():
    ...
    models_config = {
        Format.ONNX: [ONNXConfig(opset=17, dynamic_axes={}, graph_surgeon_optimization=False)],
    }
    pipeline = torch_export_builder(config=config, models_config=models_config)
