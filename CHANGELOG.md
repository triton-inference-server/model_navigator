...
- Model Navigator CLI:
  ...
  - A new option `--model-config-name` has been added to the `select` command.
  ...
  - The `--tensorrt-strict-types` option has been removed due to deprecation of the functionality in upstream libraries.
- Conversion related:
  - Converter accept passing target device kind to selected CPU or GPU supported conversions
- Triton Model configuration related:
  - Added support for OpenVINO accelerator for ONNXRuntime
- Bugfixes
